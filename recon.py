...
def compare_tomo(tomo,true_obj):
...
def recon():
    ...
    #times_loop['outfile']+=time_saving
    times_loop['write']+=time_saving
    times_loop['tot']=time_tot
    ...
    if time_file == 1:
        ...
        f.write('\nTomogram shape = (%d, %d, %d) \nNumber of angles = %d \nMPI size = %d' % (num_slices, num_rays, num_rays, num_angles, mpi_size))
        f.write('\nLoop time = %f, Setup time = %f, Saving time = %f, Total time = %f\n\n' % (times_loop['loop'], times_loop['setup'], times_loop['tot'], time_tot))
        f.write('--------------------------------------------------------------------\n')
        """
