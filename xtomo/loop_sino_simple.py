...
# defaults
DDopts={'algo':'iradon', 'GPU': 0, 'shmem':1, 'max_chunk_slice': 16, 'verbose':1, 'max_iter':10, 'tol': 1e-3, 'file_out':'*', 'reg':.0125, 'tau':.05, 'ringbuffer':0, 'ncore':None, 'chunks': None, 'Positivity': False ,'cgsmaxit':4}
...
def reconstruct(sino, theta, rot_center, Dopts, order='sino'):
    ...
    tomo_out, times_loop = recon(sino, theta, algo = algo, tomo_out=None,
              ...
              ncore=ncore, crop=chunks, mpring=ringbuffer, Positivity = Positivity, cgsmaxit = Dopts['cgsmaxit'])

    return tomo_out        
    
  

def recon(sino, theta, algo = 'iradon', tomo_out=None, rot_center = None, max_iter = 10, tol=5e-3, GPU = True, shmem = False, max_chunk_slice=16,  reg = None, tau = None, verbose = verboseall,ncore=None, crop=None, mpring=False, Positivity = False, cgsmaxit=4):
    ...
    reconstruct=wrap(sino.shape,theta,rot_center,algo,xp=xp, obj_width=obj_width, max_iter=max_iter, tol=tol, reg=reg, tau=tau, ncore=ncore, verbose=verbose, Positivity = Positivity, cgsmaxit=cgsmaxit)   
